...
app.controller("PackageListingController", ["$scope", "$location", "$http", '$filter', 'rvServices', '$timeout', function($scope, $location, $http, $filter, rvServices, $timeout) {
        ...
        $scope.packageCategoryTypeLimit = 9;
        ...
        /* Added By mangal For key of new URL Start 12/10/2018*/
        
         var myKeydes = '';  var myKeycat = '';
        var myKeydesFinal = '';var myKeycatFinal = '';
        var holidays = window.location.href.split("/holidays/");
        
        var holidays1 = holidays[1].split("/").reverse();
        
        if(holidays1[1] == 'category'){
           
        var holidaysC = window.location.href.split("/holidays/category/");
      
        var holidaysC1 = holidaysC[1].split("/").reverse();
        var holidaysC2 = holidaysC1[0].split("?");
        var holidaysC3 = holidaysC2[0].split("-");
        for(var m=0;m<holidaysC3.length;m++ ){
            if(m<(parseInt(holidaysC3.length)-2)){
           myKeycat += holidaysC3[m].charAt(0).toUpperCase() + holidaysC3[m].slice(1)+' ';
        }
        }
        myKeycatFinal = myKeycat.trim();
        }else{
        
        var holidays2 = holidays1[0].split("?"); 
        var holidays3 = holidays2[0].split("-");
        for(var m=0;m<holidays3.length;m++ ){
            if(m<(parseInt(holidays3.length)-2)){
           myKeydes += holidays3[m]+' ';
        }
        }
       var myKeydesFinal = myKeydes.trim();
       }
        /* Added By mangal For key of new URL End 12/10/2018*/
        //console.log(Keydes);
        if (dvc === 'm')
          $scope.packageInclusionLimit = 4;
        else
          $scope.packageInclusionLimit = 4;
         
        $scope.InclIcon, $scope.ThemIcon, $scope.filterCat , $scope.filterDest , $scope.filterNight , $scope.filterPrice , $scope.tempArr = [];
        $http({method: 'GET', url: conf.SITEURL + 'tours/index/getsearchdata?key='+myKeydesFinal , headers: {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET ', 'Access-Control-Allow-Headers': 'Content-Type, X-Requested-With', }, /*cache: true*/}).then(function successCallback(response) {
            
            var des = ($scope.getUrlParameter('key')) ? $scope.getUrlParameter('key') : '';
            var noofday = ($scope.getUrlParameter('noofday')) ? $scope.getUrlParameter('noofday') : '';
            var pricerange = ($scope.getUrlParameter('pricerange')) ? $scope.getUrlParameter('pricerange') : '';
           var category = (myKeycatFinal) ? myKeycatFinal : '';
            
            //$scope.resultset = $scope.resultsetFull = response.data.rows;//$scope.searchInResponse( response.data.rows , des.split('+').join(' ') , noofday , pricerange , category ); // get rows here
            ...
            $scope.imgIndex = response.data.imgIndex; // filter array
            $scope.InclIcon = response.data.InclIcon; // filter array
            $scope.ThemIcon = response.data.ThemIcon; // filter array
            $scope.tempMyMparr = response.data.MyMparr; // filter array
...
//            console.log($scope.filterNight.length);
            
//            $scope.applyFilterBoxSize( 'night' , $scope.filterNight.length );
//            $scope.applyFilterBoxSize( 'cat' , $scope.filterCat.length );
//            $scope.applyFilterBoxSize( 'dest' , $scope.filterDest.length );
//            $scope.applyFilterBoxSize( 'price' , $scope.filterPrice.length );
        });
...
//            $scope.showLoader('result_area'); // show loader
            ...
            angular.element('.box-filter input:checkbox').each(function(){
                if($(this).is(":checked")) {
...
//          
            ...
            if( (('pr' in $scope.tempPriceArray[PkgSysId][pTypeChar][categoryName])== true ) ) {
                ...
                return $http.get(conf.SITEURL + 'public/data/package_price.json').then(function(data) {
                    var rt, tpid = '';
                    var $price;
                    console.log($scope.tempMyMparr); data.data;
                    rt = rvServices.rvFilterArrayByKey(PkgSysId, $scope.tempMyMparr);
...
//            $location.path('/ranvir'+PkgSysId);
        ...
        $scope.ngActivityTab = function() {
            ...
            $http({method: 'GET', url: conf.SITEURL + 'public/data/dynamic/activities.json' ,
                ...
                var des = ($scope.getUrlParameter('des')) ? $scope.getUrlParameter('des') : '';
                var pricerange = ($scope.getUrlParameter('pricerange')) ? $scope.getUrlParameter('pricerange') : '';
...
//                                console.log( $scope.filterPriceAct );
    ...
    //        start : code for filter activity data 
        ...
        // function is used for filtering package result data
        $scope.filterDataActivity = function() {

//            $scope.showLoader('result_area'); // show loader

            // declare the variable here
            var filterSTR = {'cat':[] , 'price' : [] };

            angular.element('.box-filter-activity input:checkbox').each(function(){
                if($(this).is(":checked")) {
            ...
            // if filter has any value
            if( (filterSTR.cat.length > 0) || (filterSTR.price.length > 0) ) {
              $scope.resultsetACT = this.filterActivityFromArray( $scope.resultsetActFull , filterSTR );
            } else {
              $scope.resultsetACT = $scope.resultsetActFull;
            }
        };
...
//        div.innerHTML = text;
        div.innerHTML = text.substring(0, 250);
...
//$('.resetfilter').on('ifChecked', function (ev) { 
//    $(ev.target).click(); 
//});
